...
async def criar_produto(
    produto: schemas.produto.ProdutoCreate, db: Session = Depends(get_db)
):
...
async def atualizar_produto(
    produto_id: int,
    produto: schemas.produto.ProdutoUpdate,
    db: Session = Depends(get_db),
):
    db_produto = controller.produto.update_produto(
        db=db, produto=produto, produto_id=produto_id
    )
    if not db_produto:
        raise HTTPException(status_code=404, detail="Produto não encontrado")
...
async def deletar_produto(produto_id: int, db: Session = Depends(get_db)):
    db_produto = controller.produto.delete_produto(
